...
Trong tiểu luận này, nhóm 9 em xây dựng chương trình theo dõi chất lượng mạng và phát hiện các khoảng thời gian có dấu hiệu suy giảm. Bài làm gồm báo cáo sáu chương và hai file Python: `collector_goc.py` để thu thập dữ liệu, `phan_tich.py` để phân tích. Báo cáo chính là **Bao_cao_T19.docx**.
...
| collector_goc.py | Do nhóm em viết; thu theo chu kỳ 30 giây, chạy liên tục, dừng bằng Ctrl+C |
| phan_tich.py | Do nhóm em viết; đọc CSV ngoại tuyến, phân tích và tạo kết quả |
| du_lieu/network_quality_data.csv | Dataset 51.070 dòng, 15 cột mà nhóm em thực sự dùng trong báo cáo |
| du_lieu/network_quality_data.log | Log do collector của nhóm em ghi, dùng để đối chiếu với CSV |
| ket_qua | Các bảng CSV, tổng hợp JSON, log phân tích và 7 biểu đồ do nhóm em tạo |
| requirements.txt | Phiên bản thư viện mà nhóm em đã dùng |

## 2. Dataset và lịch đo do nhóm em thu thập

Dataset của nhóm em gồm 51.070 bản ghi, từ 19/08/2026 17:34:00 đến 08/09/2026 01:00:00, UTC+7. Nhóm em thu trên máy Windows, kết nối Ethernet. Target có nhãn `google` do nhóm em cấu hình: ping 8.8.8.8, phân giải www.google.com, gửi HTTPS tới www.google.com. Đây là các phép đo liên quan nhưng không hoàn toàn cùng một đích IP - điểm này nhóm em đã phân tích kỹ ở Chương 2.

Nhóm em dùng Task Scheduler quản lý giờ hoạt động từ 03:00 đến 01:00 hôm sau, còn collector do nhóm em viết quản lý chu kỳ đo. Khi kiểm tra lại dữ liệu, nhóm tách được 20 phiên, ngăn cách bởi 19 khoảng nghỉ ban đêm. Trong 4.543 mốc thiếu trên lưới 30 giây, nhóm em xác định 4.541 mốc ngoài lịch và hai mốc trong lịch: 22/08 lúc 20:35:30 và 20:54:30. Mẫu đúng 01:00 vẫn được nhóm em tính trong lịch.

Khi làm sạch, nhóm em phát hiện năm lượt còn thiếu một hoặc nhiều chỉ số chính và ba lượt có thông tin chưa nhất quán; nhóm em xuất các dòng này ra file riêng để xem lại, em không thay dữ liệu thiếu thành 0 và không tự coi thởi gian ngừng đo là mạng tốt hay mạng lỗi.
...
Nhóm em cố định cấu hình chính trong mã: Google, Isolation Forest, cửa sổ 5 phút, contamination 0,05, 100 cây, tối đa 256 mẫu/cây, seed 42. Chương trình của nhóm em chỉ có hai tùy chọn cho vị trí file là `--input` và `--output`.
...
Ngưởi dùng mở `collector_goc.py` và sửa hằng số `OUTPUT_CSV` sang một **file mới**. Bản nhóm em cung cấp đang giữ nguyên đường dẫn máy ban đầu: `E:\Python\Mangmaytinh (1)\network_quality_data.csv`. Có thể sửa thành `du_lieu/thu_moi.csv` khi chạy từ thư mục bài; thư mục `du_lieu` phải tồn tại. Collector của nhóm em chưa tự tạo thư mục cha và sẽ nối thêm vào file nếu file đã có.
